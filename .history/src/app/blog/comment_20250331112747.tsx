...
const processComments = (allComments: CommentPost[]): CommentPost[] => {
  ...
  allComments.forEach((originalComment) => {
    const comment = { 
      ...originalComment,
      children: [] as CommentPost[] 
    };
  ...
  allComments.forEach((originalComment) => {
    ...
    if (originalComment.parent === 0) {
      rootComments.push(comment);
    } else {
      const parentComment = commentMap.get(originalComment.parent);
      if (parentComment) {
...
function countAllComments(comments: CommentPost[]): number {
  let count = 0;
