...
// Tách hàm xử lý comments thành một hàm riêng để tái sử dụng
const processComments = (allComments: any[]): any[] => {
  ...
  // Sắp xếp comments gốc mới nhất lên trên
  rootComments.sort(
    (a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()
  );

  // Sắp xếp replies theo thứ tự thời gian
  const sortChildrenComments = (comments: any[]) => {
...
export default function CommentsPost({ slug }: { slug: string }) {
  ...
  // Tách fetch comments thành một hàm riêng
  ...
  const handleSubmitCommentForm = async (
    ...
    try {
      const commentData = {
        ...
        author_email: email,
        content: content,
        parent: replyingTo ? parseInt(String(replyingTo)) : 0
      };
      ...
      await response.json();
      await fetchComments(); // Gọi lại hàm fetchComments đã tách
      setReplyingTo(null);
  ...
  // Tách thành component riêng ở cấp cao hơn
