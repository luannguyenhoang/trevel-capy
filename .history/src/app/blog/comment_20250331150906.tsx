...
const CommentReplyInput = React.memo(({ onSubmit }: { onSubmit: (content: string) => void }) => {
        ...
        {depth > 0 && (
          <Box
            ...
            _before={{
              ...
              width: "46px",
              height: "15px",
              borderLeft: "2.5px solid #90CDF4",
              borderBottom: "2.5px solid #90CDF4",
            ...
            {hasChildren && showReplies && (
              <Box mt={2} ml={8} position="relative">
                {comment.children.map((child) => (
                  <Box key={child.id} mb={2}>
                    <CommentItem
                      comment={child}
                      depth={depth + 1}
                      replyingTo={showReplies ? replyingTo : null}
                      replyingToName={replyingToName}
                      onReply={onReply}
                      onSubmitComment={onSubmitComment}
                      onCancelReply={onCancelReply}
                      parentExpanded={showReplies}
                    />
                  </Box>
                ))}
                {showReplies && replyingTo === comment.id && (
                  <Box mt={2}>
                    <CommentReplyInput 
                      onSubmit={(content) => onSubmitComment("", content)} 
                    />
                  </Box>
                )}
              </Box>
