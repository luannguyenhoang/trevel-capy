  ...
  const formatMessageContent = (content: string) => {
    ...
    if (content.includes("```")) {
      ...
      return segments.map((segment, index) => {
        ...
        // Regular text - improve text formatting with proper line breaks
        return <Text key={index} whiteSpace="pre-wrap">{segment}</Text>;
      });
    }
  ...
  return (
    <Box bg="gray.50" minH="100vh" py={4}>
      <Container maxW="container.lg" h="calc(100vh - 2rem)">
        ...
        <VStack h="100%" spacing={4} bg="white" borderRadius="xl" boxShadow="lg" overflow="hidden">
          <Flex 
            w="100%" 
            bg="blue.500" 
            p={4} 
          ...
          >
            {messages.length === 0 ? (
              ...
              >
                ...
                <Text fontSize="lg" textAlign="center">Start a conversation with the AI assistant</Text>
                <Text fontSize="sm" mt={2} textAlign="center">Ask anything about coding, writing, or general questions</Text>
              </Flex>
            ) : (
